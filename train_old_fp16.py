...
def train(output_directory, log_directory, checkpoint_path, warm_start, n_gpus,
    ...
    scaler = GradScaler('cuda', enabled=hparams.fp16_run, init_scale=2**16, growth_interval=100)
    ...
    for epoch in range(epoch_offset, hparams.epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            optimizer.zero_grad(set_to_none=True) # So that I can train faster duhh
            x, y = model.parse_batch(batch)
            text_padded, input_lengths, mel_padded, max_len, output_lengths,raw_text, *_ = x

            with autocast('cuda', enabled=hparams.fp16_run):
                y_pred = model(x)
                mel_out, mel_out_postnet, gate_out, alignments, tp_gst_output, *_ = y_pred
            ...
            if is_overflow and rank==0:
                # or: if scaler.is_enabled() and scaler.get_scale() == 65536.0 and ... etc.
